...
function Cart() {
    const { itemsInCart, getCartItems, addItemToCart, removeItemFromCart, reduceItemQuantity } = useContext(CartContext);
                ...
                <div className="order-items flex flex-col text-left w-[3/5]">
                    ...
                    {itemsInCart?.length === 0 ? (<p>No items in cart</p>) : (
                        <>
                            {
                                ...
                                const { title, qty, subCategory, price, weight, id, _id, imageURL } = item;

                                return (
                                    <div className="flex justify-around border-b-2 items-center" key={id}>
                        
                                        <div className="grow">
                                            <NavLink to={`/product-detail/${_id}`} className=" flex items-center">
                            <img src={imageURL} alt={`${title} thumbnail`} className="h-[100px] w-[100px]"/>
                        ...
                        <span className="w-1/5">
                            <button className="px-1 bg-blue-900 text-white py-1 px-2 rounded-lg mx-2 min-w-[25px] font-bold" onClick={() => addItemToCart(item)}>+</button>
                            <span>{qty}</span>
                            <button className="px-1 bg-blue-900 text-white py-1 px-2 rounded-lg mx-2 min-w-[25px] font-bold" onClick={() => reduceItemQuantity(item._id, qty) }>-</button>
                        </span>
                                        <span className="w-1/5">Rs. {qty * price}</span>
                                        <span className="w-1/5 flex flex-col"><button className="rounded-full bg-red-700 text-white hover:bg-white hover:text-red-700 m-3 px-2 py-1" onClick={() => removeItemFromCart(item._id)}>Delete</button>
                                        <button className="rounded-full bg-red-700 text-white hover:bg-white hover:text-red-700 px-2 py-1">Move to wishlist</button></span>
                    </div>
                                )
                            })
                            }
